...
const dogsIdRaza = function(req, res){
    ...
    if(razaId<265){
        ...
        razaId = razaId -264
        Dog.findOne({include: Temperamento, where:{Id: razaId}}).then(resultado =>{
            if(resultado){
                let temperamento = '';
                resultado.temperamento.forEach(i => {
        ...
        .catch(err => {return res.status(404).send('No se encontro la raza')})
    }
}
